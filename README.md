...
3. **裝訂與匯出 PDF**：三者共用底部「檔名／前綴」文字框，直覺並列三種匯出動作按鈕：
   `[ 匯出全部 PDF ] [ 另存選取頁面 ] [ 拆分選取頁面 ]`
   - **✨ 匯出全部 PDF**：永遠將畫布全部頁面依現有順序合併為 1 個 PDF（100% 向量無損合成）。
   - **📑 另存選取頁面**：有選取時僅將選取的頁面合併為 1 個 PDF；無選取時則預設將全部頁面合併為 1 個 PDF（依畫布順序排列）。
   - **📄 拆分選取頁面**：有選取時僅將選取的頁面各自輸出為獨立單頁 PDF；無選取時則將全部頁面各自輸出為獨立單頁 PDF。
